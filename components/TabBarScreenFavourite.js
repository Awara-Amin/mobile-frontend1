...
import * as actions from "../assets/Redux/Actions/favActions";
...
const TabBarScreenFavourite = (props) => {
  const context = useContext(AuthGlobal);
  // console.log("I need userrrrrrrrr1 kaka");
  // console.log(context);
  ...
  useEffect(() => {
    ...
    // setTotalPrice();
    // };
  }, [props, propedFavItem, forRendering]);
        ...
        <View>
          ...
          <View style={styles.bookContainerStyle}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                // alert("hi kaka");
                [props.removeFromFav(item), setForRendering(true)];
              }}
              // onPress={() =>
              //   navigation.navigate("TimeSlots", {
              //     image: item.image,
...
const mapToDispatchToProps = (dispatch) => {
  return {
    clearFav: () => dispatch(actions.clearFav()),
    removeFromFav: (item) => dispatch(actions.removeFromFav(item)),
    addItemToFav: (product, qty) =>
      dispatch(actions.addToFav({ quantity: qty, product })),
  };
};
